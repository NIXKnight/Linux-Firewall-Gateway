...
- Server processes: `nfsdcld`, `rpc.nfsd`, and `exportfs` only. The entrypoint does not launch `rpcbind`, `rpc.mountd`, or `rpc.statd`.
...
The entrypoint fails closed when `/proc/fs/nfsd`, `nfsdcld`, `rpc.nfsd`, `exportfs`, or `/etc/exports` is unavailable. It creates persistent state directories, mounts the kernel NFS control filesystem and `rpc_pipefs` inside the privileged container, starts `nfsdcld` for kernel client-recovery upcalls, starts the configured NFSD threads, loads exports, and remains alive while checking the thread count and recovery daemon. SIGTERM unexports, stops the kernel server, and stops `nfsdcld` before exit.

The health probe is read-only: it requires a positive NFSD thread count, a live `nfsdcld` process with its persistent database, and a readable export table. It does not start services.
...
docker run --rm --entrypoint /bin/sh nixknight/nfs-kernel:local -c 'command -v nfsdcld && command -v rpc.nfsd && command -v exportfs && ! pgrep -x rpcbind && ! pgrep -x rpc.mountd && ! pgrep -x rpc.statd'
```
