...
docker run --rm --entrypoint /bin/sh nixknight/nfs-kernel:local -c 'command -v rpc.idmapd && command -v nfsdcld && command -v rpc.nfsd && command -v exportfs && grep -Fxq "Domain = h.nixknight.pk" /etc/idmapd.conf && grep -Fxq "Nobody-Group = nogroup" /etc/idmapd.conf && ! pgrep -x rpcbind && ! pgrep -x rpc.mountd && ! pgrep -x rpc.statd'
```
